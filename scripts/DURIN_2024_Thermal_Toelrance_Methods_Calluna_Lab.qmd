...
DURIN_data <- read_excel(path = here("raw_data/CV_methods_2024.xlsx"),sheet = "CV_Methods_Oct2024",col_names = TRUE,trim_ws = TRUE)
...
```{r}
# Marine : this chunck doesn't work, (error : FUN argument missing). I tried to change some lines but it still doesn't work
...
#Marine : replace DURIN_Methods$TempExpTime by reform_dataset$PlantSampleID
T50metrics <- psiiht(Temperature = DURIN_data_subset$TempExp,
                     FvFm = DURIN_data_subset$FvFm_Final,
                     control.temp = 20,
                     id = reform_dataset$PlantSampleID,# this is unique per sample
                     TempExpTime = DURIN_data_subset$TempExpTime,
                     # plant_nr = DURIN_data_subset$plant_nr,
                     plot.est = TRUE,
                     boots = 9) 
